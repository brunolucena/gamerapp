...
const StoreOrderStatus = () => {
  ...
  const {myOrder, storeOrders} = useSelector(
    (state: GamerAppReduxStore) => state,
  );

  const {loading, myOrderStatus} = myOrder;
  const {activeStoreOrder} = storeOrders;

  useEffect(() => {
    dispatch(getMyOrderStatus({myOrderId: activeStoreOrder.myOrderId}));
  }, [activeStoreOrder.myOrderId, dispatch]);
