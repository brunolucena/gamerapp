...
const MyOrdersPending = () => {
  const dispatch = useDispatch();
  const navigation = useNavigation();

  const {myOrderList, user} = useSelector((state: GamerAppReduxStore) => state);

  const {loading, myOrdersPending: myOrders} = myOrderList;
  const {gamerId} = user.user;

  useEffect(() => {
    dispatch(getMyOrdersPending({finishedOnly: false, gamerId}));
  }, [dispatch, gamerId]);
  ...
  function renderEmptyContent() {
    return (
      <View margin-20>
        {!loading && <Soldier text="Ops, você não tem compras pendentes." />}
      </View>
    );
  }
